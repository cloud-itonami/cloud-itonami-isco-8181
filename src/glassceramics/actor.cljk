(ns glassceramics.actor
  "GlassCeramicsPlantCoordActor — the ISCO-08 8181 glass and ceramics
  plant operator plant scheduling/logistics coordination actor as a
  ...
  cloud-itonami-isco-8114's mineralplant.actor for the
  ...
  The unconditional invariant: the Glass and Ceramics Plant
  Coordination Advisor can never directly commit an operating record
  the GlassCeramicsPlantCoordGovernor refuses, and can never make this
  actor finalize a plant-operation-execution decision (e.g. deciding
  to run or proceed with a specific glass-melting furnace or ceramics
  kiln cycle) or a plant-safety-clearance decision (e.g. declaring the
  plant floor safety-cleared), or override a plant safety officer's
...
(defn build-graph
  "Build a compiled GlassCeramicsPlantCoordActor graph. `store`
  implements `glassceramics.store/Store`. `advisor` implements
  `glassceramics.advisor/Advisor` (defaults to `mock-advisor`).
